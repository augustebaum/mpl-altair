def _allowed_ranged_marks(enc_channel, mark):
    """TODO: DOCS"""
    return (
        mark in ["area", "bar", "rect", "rule"] if enc_channel in ["x2", "y2"] else True
    )
...
def _convert(chart):
    ...
    for enc_channel in chart.encoding:
        if not _allowed_ranged_marks(enc_channel, chart.mark):
            raise ValueError(
                "Ranged encoding channels like x2, y2 not allowed for Mark: {}".format(
                    chart.mark
                )
            )

    for k, channel in chart.encoding.items():
        mapping[_mappings[k](channel.type, channel.data)[0]] = _mappings[k](

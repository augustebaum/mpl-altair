...
def _handle_line(chart, ax):
    ...
    try:
        for label, subset in chart.data.groupby(list_fields(chart, groups)):
            ...
            ax.plot(
                subset[chart.encoding["x"].field],
                subset[chart.encoding["y"].field],
                **kwargs,
            )
    except ValueError:
...
def _opacity_norm(chart, val):
    ...
    desired_min, desired_max = (
        0.15,
        1,
    )  # Chosen so that the minimum value is visible (aka nonzero)
    return ((val - data_min) / (data_max - data_min)) * (

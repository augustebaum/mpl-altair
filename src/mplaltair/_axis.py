...
def _set_scale_type(channel, ax):
    ...
    if channel.scale["type"] == "log":
        ...
        if channel.name == "x":
            ...
            lims["left"] = base ** np.floor(
                np.log10(channel.data.min()) / np.log10(base)
            )
        else:  # y-axis
            ax.set_yscale("log", basey=base)
            # lower limit: round down to nearest major tick (using log base change rule)
            lims["bottom"] = base ** np.floor(
                np.log10(channel.data.min()) / np.log10(base)
            )
        ...
        raise NotImplementedError(
            "sequential scales used primarily for continuous colors"
        )
    else:
        raise NotImplementedError
    return lims
    ...
    if "values" in channel.axis:
        if channel.type == "temporal":
            current_axis[channel.name].set_major_locator(
                ticker.FixedLocator(_convert_to_mpl_date(channel.axis.get("values")))
            )
        elif channel.type == "quantitative":
            current_axis[channel.name].set_major_locator(
                ticker.FixedLocator(channel.axis.get("values"))
            )
        else:
            raise NotImplementedError
        ...
        current_axis[channel.name].set_major_locator(
            ticker.MaxNLocator(
                steps=[2, 5, 10],
                nbins=channel.axis.get("tickCount") + 1,
                min_n_ticks=channel.axis.get("tickCount"),
            )
        )
...
def _set_tick_formatter(channel, ax):
    ...
    if channel.type == "temporal":
        ...
        current_axis[channel.name].set_major_formatter(
            mdates.DateFormatter(format_str)
        )  # May fail silently

    elif channel.type == "quantitative":
        if format_str:
            current_axis[channel.name].set_major_formatter(
                ticker.StrMethodFormatter("{x:" + format_str + "}")
            )

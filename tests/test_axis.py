...
def test_axis_more_than_x_and_y():
    chart = (
        alt.Chart(df_quant)
        .mark_point()
        .encode(alt.X("a"), alt.Y("b"), color=alt.Color("c"))
    )
...
@pytest.mark.parametrize(
    ...
    [
        (df_quant, "a", "c"),
        (df_quant, "neg", "alpha"),
        (df_temp, "months", "years"),
        (df_temp, "years", "months"),
        (df_temp, "months", "combination"),
    ],
)
...
def test_axis_zero_quantitative(x, y, zero):
    chart = (
        ...
        .mark_point()
        .encode(
            alt.X(x, scale=alt.Scale(zero=zero)), alt.Y(y, scale=alt.Scale(zero=zero))
        )
    )
    fig, ax = convert(chart)
    return fig
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            "months",
            [
                alt.DateTime(year=2015, month=1, date=12),
                alt.DateTime(year=2015, month=4, date=18),
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (df_temp, "hrs", 4),
        (df_temp, "hrs", 9),
        (df_temp, "combination", 1),
        (df_temp, "combination", 4),
        (df_temp, "combination", 9),
    ],
)
...
@pytest.mark.mpl_image_compare(baseline_dir="baseline_images/test_axis")
@pytest.mark.parametrize(
    "df,column,scale_type", [(df_quant, "log", "log"), (df_temp, "years", "time")]
)
def test_axis_scale_basic(df, column, scale_type):
    chart = (
        alt.Chart(df)
        .mark_point()
        .encode(alt.X(column, scale=alt.Scale(type=scale_type)), alt.Y("a"))
    )
    fig, ax = convert(chart)
    return fig
...
@pytest.mark.mpl_image_compare(baseline_dir="baseline_images/test_axis")
@pytest.mark.parametrize(
    "column,type,base,exponent", [("log", "log", 10, 1), ("log2", "log", 2, 1)]
)
...
@pytest.mark.mpl_image_compare(baseline_dir="baseline_images/test_axis")
@pytest.mark.parametrize(
    "column,type,base,exponent", [("log", "log", 10, 1), ("log2", "log", 5, 1)]
)
...
def test_axis_scale_NotImplemented_quantitative(df, x, type):
    chart = (
        alt.Chart(df)
        .mark_point()
        .encode(alt.X(x, scale=alt.Scale(type=type)), alt.Y("a"))
    )
    convert(chart)

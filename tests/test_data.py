...
df_nonstandard = pd.DataFrame(
    {
        ...
        "e": pd.to_datetime(
            ["1/4/2016 10:00", "5/1/2016 10:10", "3/3/2016"], format="mixed"
        ),
    }
)
...
def test_convert_to_mpl_str():
    assert list(_data._convert_to_mpl_date(df_nonstandard["c"].values)) == list(
        mdates.datestr2num(df_nonstandard["c"])
    )


def test_convert_to_mpl_datetime64():
    assert list(_data._convert_to_mpl_date(df_nonstandard["e"].values)) == list(
        mdates.date2num(df_nonstandard["e"])
    )


def test_convert_to_mpl_altair_datetime():
    dates = [
        alt.DateTime(year=2015, date=7).to_dict(),
        alt.DateTime(year=2015, month="March", date=20).to_dict(),
    ]
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            alt.DateTime(year=2015, month="March", date=7).to_dict(),
            mdates.datestr2num("2015-03-07"),
        ),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            alt.DateTime(year=2015, date=7, milliseconds=1).to_dict(),
            "2015-01-07 00:00:00.001",
        ),
        pytest.param(
            alt.DateTime(day="Mon").to_dict(),
            "2015-01-07",
            marks=pytest.mark.xfail(raises=NotImplementedError),
        ),
        pytest.param(
            alt.DateTime(year=2015, date=20, quarter=1).to_dict(),
            "2015-01-20",
            marks=pytest.mark.xfail(raises=NotImplementedError),
        ),
        pytest.param(
            alt.DateTime(year=2015, date=20, utc=True).to_dict(),
            "2015-01-07",
            marks=pytest.mark.xfail(raises=NotImplementedError),
        ),
        pytest.param(
            alt.DateTime(date=20).to_dict(),
            "2015-01-07",
...
def test_altair_datetime(date, expected):
    assert mdates.date2num(
        _data._altair_DateTime_to_datetime(date)
    ) == mdates.datestr2num(expected)

...
@pytest.mark.xfail(
    raises=ValueError, reason="It doesn't make sense to have x2 and y2 on scatter plots"
)
def test_quantitative_x2_y2():
    chart = ChartMetadata(
        alt.Chart(df_quant)
        .mark_point()
        .encode(alt.X("a"), alt.Y("b"), alt.X2("c"), alt.Y2("alpha"))
    )
    _convert(chart)
...
@pytest.mark.parametrize(
    "channel,dtype", [("quant", "quantitative"), ("ord", "ordinal")]
)
def test_convert_color_success(channel, dtype):
    chart_spec = ChartMetadata(
        alt.Chart(df).encode(color=alt.Color(field=channel, type=dtype)).mark_point()
    )
    mapping = _convert(chart_spec)
...
def test_convert_fill(channel, type):
    chart_spec = ChartMetadata(
        alt.Chart(df).encode(fill="{}:{}".format(channel, type)).mark_point()
    )
    mapping = _convert(chart_spec)
    assert list(mapping["c"]) == list(df[channel].values)
...
@pytest.mark.xfail(
    ...
    reason="The marker argument in scatter() cannot take arrays",
)
...
@pytest.mark.xfail(
    raises=NotImplementedError,
    reason="The alpha argument in scatter() cannot take arrays",
)
...
@pytest.mark.xfail(
    raises=NotImplementedError, reason="Dates would need to be normalized for the size."
)
@pytest.mark.parametrize("column", ["years", "months", "days", "hrs", "combination"])
...
@pytest.mark.xfail(
    raises=NotImplementedError, reason="Stroke is not well supported in Altair"
)
...
@pytest.mark.xfail(
    raises=NotImplementedError, reason="Stroke is not well defined in Altair"
)
@pytest.mark.parametrize("column", ["years", "months", "days", "hrs", "combination"])
...
@pytest.mark.xfail(
    raises=NotImplementedError, reason="Aggregate functions are not supported yet"
)
...
chart_quant = (
    ...
    .mark_point()
    .encode(
        alt.X(field="a", type="quantitative"),
        alt.Y("b"),
        alt.Color("c:Q"),
        alt.Size("s"),
    )
...
chart_fill_quant = (
    alt.Chart(df_quant)
    .mark_point()
    .encode(alt.X(field="a", type="quantitative"), alt.Y("b"), alt.Fill("fill:Q"))
)
...
def test_scatter_temporal(channel):
    chart = (
        alt.Chart(df)
        .mark_point()
        .encode(alt.X("years"), alt.Y("quantitative"), channel)
    )
    fig, ax = convert(chart)
    return fig
...
class TestLines(object):
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("a:Q", "b:Q", "d:Q"),
            pytest.param(
                "a:N", "b:N", "c:N", marks=pytest.mark.xfail(raises=NotImplementedError)
            ),
            pytest.param(
                "a:O", "b:O", "c:O", marks=pytest.mark.xfail(raises=NotImplementedError)
            ),
        ],
    )
    def test_line_stroke(self, x, y, s):
        chart = alt.Chart(df_line).mark_line().encode(alt.X(x), alt.Y(y), alt.Stroke(s))
        fig, _ = convert(chart)
        return fig
    ...
    def test_line_color(self):
        chart = (
            alt.Chart(df_line)
            .mark_line()
            .encode(alt.X("a"), alt.Y("b"), alt.Color("d"))
        )
        fig, _ = convert(chart)
        return fig
    ...
    def test_line_opacity(self, o):
        chart = (
            alt.Chart(df_line)
            .mark_line()
            .encode(alt.X("a"), alt.Y("b"), alt.Opacity(o))
        )
        fig, ax = convert(chart)
        return fig
    ...
    @pytest.mark.mpl_image_compare(baseline_dir="baseline_images/test_convert")
    @pytest.mark.parametrize(
        "c,o", [("d:Q", "d:Q"), ("c:N", "c:O"), ("dates:T", "dates:T")]
    )
    def test_line_opacity_color(self, c, o):
        chart = (
            alt.Chart(df_line)
            .mark_line()
            .encode(alt.X("a"), alt.Y("b"), alt.Color(c), alt.Opacity(o))
        )
        fig, ax = convert(chart)
        return fig
